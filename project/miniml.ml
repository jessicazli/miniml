...
let repl () =
  ...
  while true do
    (try
        ...
        (* EVALuate it *)
        let res_s, res_d, res_l = Ev.val_to_exp (Ev.evaluate_s exp env), 
                                  Ev.val_to_exp (Ev.evaluate_d exp env), 
                                  Ev.val_to_exp (Ev.evaluate_l exp env) in
        ...
        printf "s==> %s\n" (Ex.exp_to_concrete_string res_s);
        printf "d==> %s\n" (Ex.exp_to_concrete_string res_d);
        printf "l==> %s\n" (Ex.exp_to_concrete_string res_l)
